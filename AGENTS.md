...
- **Experimental gate** (`experimental.enabled`, default false) is all-or-nothing. It gates the Small-LLM profile and the E2S execution mode; disabling it leaves stored Small-LLM config intact but makes the profile ineffective and hides its settings tab. RESEARCH is always available and is not gated by this switch. `backend/frontend_api_research.go` owns RESEARCH activation, workspace containment, non-destructive skill seeding, and watcher setup.
...
- **E2S mode** (`e2s.*`, experimental, default off): a per-message alternative execution loop where the model's only memory is an externalized state Σ it patches every turn (`e2s_step` meta-tool: `state_patch` + `action`) — every turn is a fresh one-shot `[system, user]` request, so context stays O(1) in turns. Own loop in `core/e2s` on raw sp4rk primitives (no Executor/Conductor); actions dispatch through `ToolRegistry.Execute` with every security gate intact; hybrid schema (8 fixed core keys, add-only extensions, byte cap, schema fingerprint); per-step Σ persistence (`task_e2s_state`) + resume; delegation via the injected conductorLauncher with an inert plan state; frontend Execution State panel replaces the plan view (`e2s_state` event). Mutually exclusive with goal mode. See `specs/domains/e2s.md` and `specs/decisions/039-e2s-explicit-execution-state.md`.
...
| `e2sStore`           | Per-session E2S execution-state Σ snapshots from `e2s_state` events (patch merge owned by the store; cleared on session switch/delete) |
