...
7. **Experimental gate.** The whole feature sits behind `experimental.enabled` (fail-closed both sides): the backend rejects `SendMessage` with `e2s=true` when the gate is off, and the frontend toggle (`E2SToggle.tsx`) is hidden. The `e2s:` config section (step budget, state byte limit, retry count, observation truncation, anti-spin thresholds) is ineffective when gated off — mirroring the Small-LLM profile's configadapter pattern.
