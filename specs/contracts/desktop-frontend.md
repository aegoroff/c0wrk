...
| `ModelProfilesSettingsResponse`      | backend  | backend → frontend | Effective Model Profiles gate (embedded in `ConfigResponse.model_profiles`): `enabled` (resolved master toggle, experimental gate folded in) + `essential_tools_enabled` (resolved `essential_tools` variant). Drives the goal-mode block — see [../domains/model-profiles.md](../domains/model-profiles.md#goal-mode-gate-goalblocked) |
...
| `UpdateExperimentalFeatures` | enabled            | error                             | Toggle the master experimental-features switch (it gates the Model Profiles feature and the E2S execution mode). Persists the change — when disabling, it also persists `model_profiles.enabled = false` (the Model Profiles master-toggle reset), so re-enabling the gate never silently reactivates the profile — rebuilds the LLM router (so the gated model profile applies immediately), and updates the session manager's Model Profiles snapshot (the active profile's values and identity). RESEARCH mode is unaffected by this switch: it is activated per project via its own RPCs (`EnableResearch`/`GetResearchStatus`/`GetResearchGraph`) |
...
| `SetModelProfilesEnabled`          | enabled                  | error                             | Toggle the global `model_profiles.enabled` master switch, persisted to config.yaml via the same Model Profiles change tail (config:updated emit + router rebuild + session-manager snapshot). Enabling fails closed while `experimental.enabled` is off; disabling is always allowed. A set matching the stored value is a no-op; a failed write rolls the in-memory value back. This is the Model Profiles settings tab's master switch — it never touches the experimental gate (that is `UpdateExperimentalFeatures`) |
...
> **Experimental-features gate**: `ConfigResponse` carries `experimental.enabled` — a single, all-or-nothing switch whose gated features are the Model Profiles feature and the E2S execution mode. When off, the Model Profiles feature is forced off in `ToBuilderConfig` and `SendMessage` rejects `e2s=true` fail-closed. Closing the gate also persists `model_profiles.enabled = false` (so re-enabling the gate never silently reactivates the Model Profiles feature). The frontend hides the corresponding affordances reactively in the same session: the Model Profiles settings tab and the E2S per-message toggle. RESEARCH mode is not gated by this switch — it is activated per project via `EnableResearch` and stays available.
>
> **Model Profiles goal gate**: `ConfigResponse.model_profiles` (`ModelProfilesSettingsResponse`: `enabled` + `essential_tools_enabled`) carries the EFFECTIVE, resolved Model Profiles state — the experimental gate is folded into `enabled` (`effectiveModelProfilesConfig`), and both fields are false when the config is not loaded. The frontend mirrors it (`modelProfilesGateStore` / `useModelProfilesGate`) to disable the goal toggle reactively; the backend is authoritative, refusing goal sends and paused-goal resumes. `goalBlocked = enabled && essential_tools_enabled` — master-on with the variant off does NOT block. See [../domains/model-profiles.md](../domains/model-profiles.md#goal-mode-gate-goalblocked).
