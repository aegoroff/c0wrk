...
| `SLMSettingsResponse`      | backend  | backend → frontend | Effective Small-LLM gate (embedded in `ConfigResponse.slm`): `enabled` (resolved master toggle, experimental gate folded in) + `essential_tools_enabled` (resolved `essential_tools` variant). Drives the goal-mode block — see [../domains/slm.md](../domains/slm.md#goal-mode-gate-goalblocked) |
...
| `ResumeTask`           | id, modelOverride, reasoningEffort | error                     | Resume failed task (honors model/reasoning override chosen before resuming); rejected for archived sessions, and refuses a paused goal while the Small-LLM essential-tools narrowing is active ([../domains/slm.md](../domains/slm.md#goal-mode-gate-goalblocked)) |
...
>
> **Small-LLM goal gate**: `ConfigResponse.slm` (`SLMSettingsResponse`: `enabled` + `essential_tools_enabled`) carries the EFFECTIVE, resolved Small-LLM state — the experimental gate is folded into `enabled` (`effectiveSLMConfig`), and both fields are false when the config is not loaded. The frontend mirrors it (`slmGateStore` / `useSLMGate`) to disable the goal toggle reactively; the backend is authoritative, refusing goal sends and paused-goal resumes. `goalBlocked = enabled && essential_tools_enabled` — master-on with the variant off does NOT block. See [../domains/slm.md](../domains/slm.md#goal-mode-gate-goalblocked).
