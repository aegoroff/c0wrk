...
| Model Profiles (tuning for local / mid-size models) | [domains/model-profiles.md](domains/model-profiles.md), [decisions/041-slm-profiles.md](decisions/041-slm-profiles.md), [decisions/022-small-llm-profile.md](decisions/022-small-llm-profile.md), [decisions/042-goal-mode-slm-essential-tools-gate.md](decisions/042-goal-mode-slm-essential-tools-gate.md) |
...
- [041-slm-profiles.md](decisions/041-slm-profiles.md) - Model Profiles catalog: research-backed read-only predefined profiles ∪ operator custom profiles (`~/.c0wrk/model-profiles.yaml`) replace the inline `small_llm.*` knobs; `config.yaml` persists only `model_profiles.enabled` + `model_profiles.active_profile` (generic soft fallback, sanctioned reset migration); five profile-scoped RPCs + the `SetModelProfilesEnabled` master-toggle RPC (persisted toggle; enabling gate-checked, closing the experimental gate resets it) + suggested-profile hint (never auto-applied); `agent_metrics` carries profile identity; Model Profiles nomenclature → Partially supersedes ADR-022
