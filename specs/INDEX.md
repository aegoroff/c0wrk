...
- [042-goal-mode-slm-essential-tools-gate.md](decisions/042-goal-mode-slm-essential-tools-gate.md) - Goal mode is refused while the SLM `essential_tools` narrowing is active (`goalBlocked = SLM.Enabled && EssentialTools.Enabled`, the effective values — master-on with the variant off does NOT block): the narrowing hides the goal-loop tooling, so rather than enter an unrunnable loop the goal is refused before any side effect; enforced at three layers (frontend gate, session API, orchestrator `ErrGoalBlockedBySLM`) and surfaced via `ConfigResponse.slm`; the goal derivation/verification passes also opt into the profile's `system_prompt` Lite variant
