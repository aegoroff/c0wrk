...
| `experimentalStore`  | Master Experimental Features gate from runtime config (`experimental.enabled`) — the SOLE gate for the Model Profiles feature (its settings tab hides when off) and the E2S execution mode (its per-message toolbar toggle hides when off); `lib/e2sGate.ts` composes this switch with the armed toggle for the fail-closed E2S send gate. The `enabled` switch is loaded from `GetConfig`, retried on `backend:ready`/`config:updated` until latched, updated in place from Settings | No           |
