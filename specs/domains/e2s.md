...
- `backend/config/config.go` + `backend/config/defaults.go` + `backend/configadapter.go` — `E2SConfig` (`e2s.*`) with defaults, gated fail-closed on `experimental.enabled` (configadapter pattern from the Model Profiles feature)
