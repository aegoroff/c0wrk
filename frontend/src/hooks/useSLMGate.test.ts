...
import { useSLMGate } from './useSLMGate'
import { useSLMGateStore } from '@/stores/slmGateStore'
import type { ConfigResponse, ConfigSLMResponse } from '@/types/models'

/**
 * Minimal valid ConfigResponse; the hook only reads loaded/slm. `llm` and
 ...
 * `slm` omitted entirely models an older payload without the field.
 */
function makeConfig(loaded: boolean, slm?: ConfigSLMResponse): ConfigResponse {
  return {
    loaded,
    log_level: 'info',
    config_errors: [],
...
/** Latest value returned by useSLMGate across re-renders. */
...
describe('useSLMGate', () => {
  ...
  it('treats a missing slm block as "off" and does not block', async () => {
  ...
  it('does NOT latch when the backend answers loaded=false during startup', async () => {
    ...
    expect(useSLMGateStore.getState().loaded).toBe(false)
    expect(rendered.blocked).toBe(false)
    expect(configMocks.getConfig).toHaveBeenCalledTimes(1)
  })
  ...
  it('stays retryable (loaded=false) when the post-ready answer is still not loaded', async () => {
    ...
    expect(useSLMGateStore.getState().loaded).toBe(false)
    expect(useSLMGateStore.getState().enabled).toBe(false)
    expect(rendered.blocked).toBe(false)
    expect(configMocks.getConfig).toHaveBeenCalledTimes(2)
  })
